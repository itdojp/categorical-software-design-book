...
仕様が正しくても、
それが実装構造へ写る途中で形が崩れると運用は壊れます。
第4章では、spec から code への対応を
「構造保存」として読みます。

本章の見せ場は、対応図と関手性レビューのチェックリストです。
保存すべきものと AI に任せてよいものを、同じ図で確認できます。
...
### 仕様→実装の関手とスキーマ関手を混同しない

本章の `Spec -> Code` は、仕様成果物を実装成果物へ対応づけるための設計レビュー上の関手です。一方、第7章補論で扱う CQL / Functorial Data Migration では、DB スキーマを小さな圏として扱い、DB インスタンスを `Schema -> Set` の関手として読む場面があります。どちらも「構造保存」を確認しますが、source / target は異なります。

混同を避けるため、Context Pack では次を分けます。

- `Spec -> Code`: Objects、Morphisms、Diagrams を Types、Functions、Tests へ写す設計委任の対応。
- `Schema -> Set`: スキーマの対象・射へ、実データの集合・関数的対応を割り当てるデータインスタンスの対応。
- schema functor: 旧スキーマから新スキーマ、または共通スキーマへの写像。ここからデータ移行検証を導く。

したがって、AI に「関手性を保て」と指示するだけでは不十分です。どの関手の source / target を固定するのか、何を保存するのか、どの検証で確認するのかを明示します。

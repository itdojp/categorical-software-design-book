...
## 第7章補論: merge conflict を Pushout で見る

バージョン管理の merge も、Pushout の直観で説明できます。
ただし本書の標準運用は、引き続き Git / GitHub / PR / CI です。
Pijul や categorical patch theory は、GitHub の置換ではありません。
AI agent が branch 上で差分を作るときに、
「何を保存すべきか」「どの解決を禁止すべきか」を考える発展的な mental model として扱います。

最小の対応づけは次のとおりです。

```text
共通祖先 A から、変更 f: A -> B と g: A -> C がある。
両方を含む D が存在し、B -> D, C -> D が整合するなら merge 成功。
D が構成できない、または意味的制約を壊すなら conflict。
```

ここで重要なのは、`D` を「とにかくコンパイルする結果」と見なさないことです。
`D` は、両方の変更を含むだけでは不十分です。
Context Pack、受入テスト、監査条件、Forbidden changes を破らない候補でなければなりません。
たとえば型エラーを消すために失敗テストを削除する、監査ログを外す、認可チェックを弱める、といった解決は merge 結果として採用しません。

### `Forbidden changes` と `change_semantics` の関係

`Forbidden changes` は、作業全体で破ってはいけない非交渉の禁止事項です。例として「監査ログを削除しない」「状態モデルを合意なく変えない」が該当します。

一方、Context Pack v2 の `change_semantics` は、差分・refactor・merge のレビュー時に使う変更意味論です。
`allowed_refactors` は、意味保存と見なせる変更です。
`forbidden_conflict_resolutions` は、衝突を消すために選んではいけない解決です。
`merge_invariants` は、merge 後にも保つ検証条件です。
つまり、`Forbidden changes` が常時の禁止境界です。
`change_semantics` は、branch 上の差分処理と conflict 解決の運用ルールです。
...
AI agent に branch 作業を委任する場合は、差分生成前に `change_semantics` を渡します。
レビューでは、生成された `D` が「両方の変更を含む」だけでなく、
`merge_invariants` と `Forbidden changes` を同時に満たすかを確認します。
Pushout という語は、ここでは比喩と対応づけです。
実務上の正しさは、CI、schema validation、受入テスト、レビューコメントの解決状態で確認します。

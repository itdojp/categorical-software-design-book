...
- 合成律（Laws）:
  ...
  - 恒等射（[Identity]({{ '/glossary/' | relative_url }}#identity)）:
    - 任意の `f: A → B` に対して `f ∘ id_A = f` が成り立つ
    - 同時に `id_B ∘ f = f` も成り立つ
...
結合律は「実装の括弧の付け替え（リファクタ）をしても意味が変わらない」ための条件です。
ソフトウェアでは、パイプラインやユースケースの分割・統合、処理段階の抽象化を繰り返します。そのため、結合律が成立する形で契約を固定すると保守性が上がります。

ミニ例を示します。

`validate`（入力検証）、`reserveInventory`（在庫引当）、`authorizePayment`（決済承認）を順に適用する処理を考える。

- 結合律で許されるのは、射の順序交換ではなく括弧の付け替え（再結合）である。
  `(authorizePayment ∘ reserveInventory) ∘ validate = authorizePayment ∘ (reserveInventory ∘ validate)`
  の両辺は、どちらも `validate` → `reserveInventory` → `authorizePayment` の順に適用する。
- 射の順序交換は一般にはできない。上記の型では `authorizePayment` の入力に `Reserved` が必要なため、
  `reserveInventory` より先に置く合成は型が成立せず、そもそも定義できない。
  さらに、型制約を失った実装境界で決済を先行できてしまうと、在庫を確保できない注文へ決済する場合がある。
  副作用の発生順や監査記録も変わり、元の合成と同じ意味にはならない。
...
1. Objects テンプレで、`Order`・`Payment`・`InventoryReservation`・`Shipment`・`AuditEvent` を整理する

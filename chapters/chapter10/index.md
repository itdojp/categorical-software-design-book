...
全文は [共通例題: 注文処理](../../docs/examples/common-example/) を参照してください。
...
+   statement: Place 済みの注文を取消すと、在庫引当は解放され、監査証跡が残る
+   verification:
...
+     - AuditEvent("CancelOrder") が存在する
+
+ - id: AT5-cancel-after-payment-rejected
+   scenario: Paid の Order を CancelOrder する
+   expected:
+     - InvalidState
```

この章では `D2` を更新し、`D4` と `AT4/AT5` を追加しています。
既存契約を壊さず、変更要求の影響範囲だけを増やすのが要点です。
...
- Diagram / Acceptance test 由来のテスト
  - `D2`、`D4`、`AT4`、`AT5` がコード差分で壊れていないかを検知する

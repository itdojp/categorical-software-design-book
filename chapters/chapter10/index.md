...
### 3) read model と監査 lineage の移行検証を追加する

CancelOrder を運用へ入れると、注文サービスの正規データだけでなく、問い合わせ用 read model や監査ログにも影響します。ここでは「取消済み注文を read model で検索できること」よりも、「その read model がどの元データと監査イベントから作られたかを追跡できること」を固定します。

Context Pack v2 では、次のように `data_contracts` へ移行検証を足します。
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td>第7章</td>
      <td>統合図式</td>
      <td>注文、在庫引当、read model、監査 lineage の整合を、統合・移行境界の問題として扱う</td>
    </tr>
    <tr>
      <td>第8章</td>
      <td>分業と配線</td>

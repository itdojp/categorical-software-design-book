...
### 5. AIエージェントへの引き渡し
- Context Pack の参照（[Context Pack v1 仕様](../spec/context-pack-v1.md)）
- 使うプロンプト（付録Bまたは章末に配置）
...
- 見出し構造がテンプレに沿っている
- 用語・記法がスタイルガイド（[用語ガイド](terminology.md)、[記法ガイド](notation.md)）に沿っている
- 共通例題（[共通例題（注文処理）](../examples/common-example/README.md)）への参照が一貫している
- 可換性（不変条件）が「検証可能な形」で記述されている（テスト観点/受入条件に落ちている）
- AI引き渡しが Context Pack v1 形式で再現できる（[Context Pack v1 仕様](../spec/context-pack-v1.md)）

...
- 参照先が複数ある場合は、公開トップページの `確認したいこと別の正本` 表と矛盾しない reader-facing な導線にする
- 形式・必須項目・語義・検証手順を章末で再説明しすぎず、必要に応じて `Context Pack v1 仕様`、`Glossary`、付録D へ明示的に戻す

## PRレビュー観点（章テンプレ遵守）

- 見出し構造がテンプレに沿っている
- 用語が [用語ガイド]({{ '/style/terminology/' | relative_url }}) に沿っている
